...
export const useSources = (notebookId?: string) => {
  ...
  const toggleSourceForChat = useMutation({
    mutationFn: async ({ sourceId, selected }: { sourceId: string; selected: boolean }) => {
      const { data, error } = await supabase
        .from('sources')
        .update({ selected_for_chat: selected })
        .eq('id', sourceId)
        .select()
        .single();
    ...
    onSuccess: () => {
      // The Realtime subscription will handle updating the cache
      console.log('Source chat selection updated');
    },
  });
  return {
    sources,
    ...
    toggleSourceForChat: toggleSourceForChat.mutate,
    isTogglingSelection: toggleSourceForChat.isPending,
  };
};
